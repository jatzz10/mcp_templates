...
class DatabaseMCPServer:
    ...
    async def execute_query(self, query: str, limit: int = 100) -> list:
        ...
        if not self.connection:
            await self.connect()
        # Ensure connection exists after attempting to connect
        if not self.connection:
            raise Exception("Database connection not established. Check DB credentials and connectivity.")
    ...
    async def validate_query(self, query: str, limit: int) -> bool:
        ...
        # Allow SELECT and SHOW queries (read-only operations)
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            if not self.connection:
                ...
                if not ok or not self.connection:
                    return {
                        "status": "unhealthy",
            ...
            # Test basic query
            _ = await self.execute_query("SELECT 1", 1)
    ...
    def _register_resources(self) -> None:
        ...
        async def database_prompts():
            ...
            try:
                prompts = {
                    "system_prompt": "You are a database assistant that can query databases and access schema information via MCP.",
                    "rules": [
                    ...
                    "output_format": "Return ONLY a single-line minified JSON object with this structure: {\"action\": \"call_tool|read_resource\", \"tool\": \"tool_name\", \"args\": {...}} or {\"action\": \"read_resource\", \"uri\": \"resource_uri\"}",
                    "examples": [
                        {
...
def main():
    """Main entry point"""
    print("🚀 Starting Database MCP Server with streamable-http transport")
    ...
    try:
        ...
        import asyncio
        connected = asyncio.run(server.connect())
        if not connected:
            print("❌ Configuration validation failed")
            print("Please check your mcp.env file and database connection")
            return
